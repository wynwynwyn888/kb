...
| `TenantBookingSettings` | `tenant_booking_settings` | tenant: `tenant_id` is required primary key | confidential configuration/contact destination | AO/AA:CRUD; OP:R; TM:R, tenant admin U | RLS enabled; direct authenticated table reads/writes denied; fixed `get_tenant_booking_settings` RPC checks `can_read_tenant`, exposes `can_manage`, and redacts staff alert fields from read-only roles | booking-settings, booking-flow | Keep workers/internal writes isolated; add mutation policy only if separately justified and tested. |
